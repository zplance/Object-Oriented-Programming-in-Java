...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		// Symbols
		fill(color(66, 244, 72));
		triangle(50, 90, 45, 100, 55, 100);
		noFill();
		rect(45, 110, 10, 10);
		...
		text("5.0+ Magnitude", 75, 200);
		text("4.0+ Magnitude", 75, 225);
		text("Below 4.0", 75, 250);
	}
	...
	private void printQuakes() 
	{
		...
		for(Marker cm: countryMarkers) {
			...
			for (Marker quake: quakeMarkers) {
				...
				if (em.isOnLand()) {
					if (((LandQuakeMarker) quake).getCountry().equalsIgnoreCase(country)) {
						QuakeCounter ++;
					}
				}
			}
